...
public class FangCounterSkill extends Skill {
    ...
    public static FangCounterSkill.Builder createFangCounterSkillBuilder() {
        return (new Builder())
                .addMotion(WeaponCategories.LONGSWORD, (item, player) -> t0001Animations.FANG_COUNTER)
                .addMotion(WeaponCategories.GREATSWORD, (item, player) -> t0001Animations.FANG_COUNTER)
                .addMotion(WeaponCategories.TACHI, (item, player) -> t0001Animations.FANG_COUNTER)
    ...
    protected final Map<EntityType<?>, Integer> maxSuperStacks = Maps.newHashMap();
    protected int blockStack;
    protected int parryStack;
    protected int defaultSuperStacks;
    ...
    public void setParams(CompoundTag parameters) {
        ...
        this.maxSuperStacks.clear();
        this.blockStack = parameters.getInt("block_stacks");
        this.parryStack = parameters.getInt("parry_stacks");
        this.defaultSuperStacks = parameters.getInt("default_super_stacks");

        CompoundTag maxStacks = parameters.getCompound("max_supers");

        for (String registryName : maxStacks.getAllKeys()) {
            EntityType<?> entityType = EntityType.byString(registryName).orElse(null);

            if (entityType != null) {
                this.maxSuperStacks.put(entityType, maxStacks.getInt(registryName));
            } else {
    ...
    public void onInitiate(SkillContainer container) {
        ...
        listener.addEventListener(EventType.SET_TARGET_EVENT, EVENT_UUID, (event) -> container.getDataManager().setDataSync(t0001SkillDataKeys.SUPER_STACKS.get(), 0));
        ...
        listener.addEventListener(EventType.TAKE_DAMAGE_EVENT_ATTACK, EVENT_UUID, (event) -> {
            if (event.getResult() == ResultType.BLOCKED) {
                ...
                if (target != null) {
                    assert event.getDamageSource().getDirectEntity() != null;
                    if (target.is(event.getDamageSource().getDirectEntity())) {
                        int stacks = event.isParried() ? this.parryStack : this.blockStack;

                        this.checkStackAndActivate(container, event.getPlayerPatch(), target, container.getDataManager().getDataValue(t0001SkillDataKeys.SUPER_STACKS.get()), stacks);
                    }
                }
            }
        }, -1);
    ...
    public void checkStackAndActivate(SkillContainer container, ServerPlayerPatch playerpatch, LivingEntity target, int stacks, int addStacks) {
        int maxStackSize = this.maxSuperStacks.getOrDefault(target.getType(), this.defaultSuperStacks);
        int stacksToAdd = stacks + addStacks;

        if (stacksToAdd < maxStackSize) {
            container.getDataManager().setDataSync(t0001SkillDataKeys.SUPER_STACKS.get(), stacksToAdd);
        } else {
            if (!this.isActivated(container)) {
                this.setDurationSynchronize(container, this.maxDuration);
            }

            container.getDataManager().setDataSync(t0001SkillDataKeys.SUPER_STACKS.get(), 0);
        }
    }
    ...
    public void drawOnGui(BattleModeGui gui, SkillContainer container, GuiGraphics guiGraphics, float x, float y, float partialTick) {
        ...
        int stacks = container.getRemainDuration() > 0 ? 0 : this.maxSuperStacks.getOrDefault(container.getExecutor().getTarget().getType(), this.defaultSuperStacks)
                - container.getDataManager().getDataValue(t0001SkillDataKeys.SUPER_STACKS.get());
