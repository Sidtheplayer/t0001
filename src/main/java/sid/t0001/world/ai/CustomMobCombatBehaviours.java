    ...
    public static final CombatBehaviors.Builder<HumanoidMobPatch<?>> AMOGUS_AXE_INNATE_SPAM = CombatBehaviors.<HumanoidMobPatch<?>>builder()
            .newBehaviorSeries(
                    CombatBehaviors.BehaviorSeries.<HumanoidMobPatch<?>>builder().weight(100.0F).canBeInterrupted(true).looping(false).cooldown(2)
                            .nextBehavior(CombatBehaviors.Behavior.<HumanoidMobPatch<?>>builder().animationBehavior(Animations.THE_GUILLOTINE).withinEyeHeight().withinDistance(0.0D, 1.0D).health(11F, HealthPoint.Comparator.LESS_RATIO))
                    ...
                    CombatBehaviors.BehaviorSeries.<HumanoidMobPatch<?>>builder().weight(40.0F).canBeInterrupted(false).looping(false).cooldown(100)
